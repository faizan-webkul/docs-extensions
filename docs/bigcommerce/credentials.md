...
<!-- TODO: capture screenshot — bigcommerce-credentials-list.png — BigCommerce Credentials list grid -->
...
| Where | What it does |
|--|--|
| [Export profile filters](./export-products) | Picks which store the export pushes to. |
| [Import profile filters](./import-products) | Picks which store the import pulls from. |
| [Standard mapping](./standard-mapping) / [Custom mapping](./custom-mapping) / [Other mapping](./other-mapping) | Mappings are saved **per credential** — different stores can have different mappings. |
